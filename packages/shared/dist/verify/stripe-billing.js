...
export async function provisionBusinessStripeBilling(input) {
    ...
    const { data: business, error: businessError } = await admin
        ...
        .select("id, stripe_customer_id, stripe_subscription_id, stripe_verification_item_id, stripe_presentation_item_id")
    ...
    const verificationItem = subscription.items.data.find((item) => item.price.id === verificationPriceId);
    const presentationItem = subscription.items.data.find((item) => item.price.id === presentationPriceId);
